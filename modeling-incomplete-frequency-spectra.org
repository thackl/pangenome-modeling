...
img_fs_coalescent <- function(K, rho1, tau1, df=FALSE) {
    spec <- (1:K)*0
    specprod1 <- (K+1 - 1:K)/(K+rho1 - 1:K)
...
rho <- .5;
tau <- 200;
y <- img_fs_coalescent(K, rho, tau, TRUE)
...
x80 <- rbinoX(N, 1:K, .8, y$frequency) %>% tbl_df %>%
    ...
    select(gene, count=value)

## binom mic
img_bm <- stan_model("img-coalescent-binom-mix-02.stan")
#img_bm <- readRDS("img-coalescent-binom-mix.rds")

d100 <- list( N=nrow(x), K=K, x=x$count, c=rep(.9999, K))
m_img_bm <- sampling(img_bm, iter = 1000, chains=4, data=d100)

d080 <- list( N=nrow(x80), K=K, x=x80$count, c=rep(.80, K))
m_img_bm <- sampling(img_bm, iter = 1000, chains=4, data=d080)

print(m_img_bm)
S <- ggs(m_img_bm)
ggs_traceplot(S) + facet_wrap(~Parameter, ncol=5, scales="free_y")
ggs_caterpillar(S)
